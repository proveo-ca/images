...
#
# Launches bin/proveo using a PTY (via script) inside samples/ to assert all 4 TUI selection and auto-filtering requirements.
...
if os_out=$(printf '\n' | script -q -c "bash -c 'cd $WORKSPACE_ROOT/samples && source $WORKSPACE_ROOT/apps/cli/public/cli/bin/proveo; choose_scope opencode > /tmp/selected_scope.txt'" /dev/null 2>&1) && [[ "$(cat /tmp/selected_scope.txt)" == "$WORKSPACE_ROOT/samples" ]]; then
...
TESTS_RUN=$((TESTS_RUN + 1))
if os_out=$(printf 'z\n' | script -q -c "bash -c 'cd $WORKSPACE_ROOT/samples && source $WORKSPACE_ROOT/apps/cli/public/cli/bin/proveo; choose_scope opencode > /tmp/selected_scope.txt'" /dev/null 2>&1) && [[ "$os_out" == *$'\a'* || "$os_out" == *$'\x07'* ]] && [[ "$(cat /tmp/selected_scope.txt)" == "$WORKSPACE_ROOT/samples" ]]; then
  TESTS_PASSED=$((TESTS_PASSED + 1))
...
TESTS_RUN=$((TESTS_RUN + 1))
if os_out=$(printf 'tui\n' | script -q -c "bash -c 'cd $WORKSPACE_ROOT/samples && source $WORKSPACE_ROOT/apps/cli/public/cli/bin/proveo; choose_scope opencode > /tmp/selected_scope.txt'" /dev/null 2>&1) && [[ "$(cat /tmp/selected_scope.txt)" == "$WORKSPACE_ROOT/samples/apps/tui" ]]; then
  TESTS_PASSED=$((TESTS_PASSED + 1))
