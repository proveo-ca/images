...
Experimental coding harness definition. This definition exposes:
...
`tests/` is not present yet; this definition currently relies on `test.sh` for smoke coverage.

## Image Names and Commands

- Default image: `proveo/aider-node:latest`
- Build override: `PROVEO_AIDER_NODE_IMAGE=example/aider-node:tag ./build.sh`
- Run override: `PROVEO_AIDER_NODE_IMAGE=example/aider-node:tag ./run.sh`
- Workspace mount: input directory mounted at `/app`

Use the definition-local scripts for the contract surface:
