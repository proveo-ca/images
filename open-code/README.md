...
- Non-root user `opencode` (UID 1001) and a hardened runtime
...
```bash
docker build -t proveo/open-code:local .
```
...
```bash
docker run -it --rm --name open-code \
  -v "$PWD":/app \
  -w /app \
  proveo/open-code:local
```

### From a monorepo subfolder
...
docker run --rm \
  ...
  proveo/open-code:local \
  run -m anthropic/claude-sonnet-4-5 "List the files in /app"
```

Any args passed after the image name are forwarded to `opencode`.
